...
export class PokerGameComponent implements OnInit {
  ...
  refreshGame(): void {
    this.gameService.getDealProgress(this.currentGame ? this.currentGame.id : 0)
      .subscribe(resp => this.isDealInProgress = resp);
    if (this.isDealInProgress === true) {
      this.gameService.refreshGame(this.currentGame ? this.currentGame.id : 0)
        .subscribe(resp => {
          this.currentGame = resp.body;
        });
    } else if (this.isDealInProgress === false) {
      this.gameService.winnerLog(this.currentGame?.id)
        .subscribe(resp => {
